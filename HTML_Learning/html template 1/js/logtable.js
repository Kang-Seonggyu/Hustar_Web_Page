...
function warning_Write (whatSignal, log_index, warning_value){
    ...
    new_logPosit.innerHTML = '뭐넣지?'

    if(whatSignal == 1){
        new_logWarning.innerHTML = `위험 - 심박수 : ${warning_value}`
    }
    if(whatSignal == 2){
        new_logWarning.innerHTML = `위험 - 혈압 : ${warning_value}`
    }
    if(whatSignal == 3){
        new_logWarning.innerHTML = `위험 - 산소포화도 : ${warning_value}`
    }
    if(whatSignal == 4){
        new_logWarning.innerHTML = `위험 - 스트레스 : ${warning_value}`
    }
}