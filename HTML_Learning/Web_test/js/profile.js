// 근무자 수
localStorage.setItem("workersMount",workers.length)
...
function addProfile() {
    ...
    new_img.src = "image/"+workers[pTagCount].name+".jpg";      // image 태그 위치값 입력
    new_img.onerror = function(){this.src = "image/account.png"}; // image 없을 시 기본 이미지 출력
...
function newpage1() {
    let workerSelect = this.id;
    console.log( parseInt(workerSelect) );
